...
int main(int argc, char **args)
{
    ...
    PetscMPIInt         size;
    PetscInt            n=100,i;
    PetscScalar         dx=1.0/n,dt=0.0000001,y[100],x,t_start=0.0,t_end=2.0;
    PetscScalar         CFL=dt/(dx*dx);
    PetscBool           restart = PETSC_FALSE;/*重启标志*/

    /*初始化*/
    PetscCall(PetscInitialize(&argc,&args,(char*)0,help));
/*
    PetscCall(PetscOptionsBegin(PETSC_COMM_WORLD,NULL,NULL,NULL));
    PetscCall(PetscOptionsGetBool(NULL,NULL,"-restart",&restart,NULL));//通过命令行判断是否要重启
